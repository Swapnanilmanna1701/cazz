...
export default function TopLinkedIn() {
  ...
  const handleJobSearch = async (e: any) => {
    e.preventDefault();
    setLoading(true);
  ...
  return (
    <div className="w-full h-100vh bg-black flex flex-col gap-8 md:p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {jobs.length &&
              jobs.map((job: JobProps, index) => (
                <Card
                  key={index}
                  className="p-4 shadow-md relative transition-all duration-500 hover:scale-105"
                >
      ...
      {jobs.length && jobs.length !== 0 && !loading ? (
        <div className="flex justify-center mt-4">
          <Button onClick={(e) => pageDown(e)} disabled={page === 1}>
            Prev
          </Button>
          <Button onClick={(e) => pageUp(e)} className="ml-8">
            Next
          </Button>
        </div>
      ) : null}
